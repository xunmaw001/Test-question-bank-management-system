...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>科目</div>
        <el-select v-model="formSearch.kemu" placeholder="请选择科目" :clearable="true">
          <el-option v-for="(item, index) in kemuOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>章节</div>
        <el-select v-model="formSearch.zhangjie" placeholder="请选择章节" :clearable="true">
          <el-option v-for="(item, index) in zhangjieOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('shitiku','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(255,215,169,1) 0%, rgba(255,164,58,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shitikuAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 20px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"10px 10px","borderColor":"#52b646","borderRadius":"8px","flexWrap":"wrap","background":"#fff","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid double solid double","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"2px solid #88d67e","cursor":"pointer","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","margin":"10px","borderRadius":"8px","background":"#fff","display":"inline-block","width":"23%","position":"relative","borderStyle":"dotted dashed solid double","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">科目:{{item.kemu}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">章节:{{item.zhangjie}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/kemu/kemu').then(res => {
        if (res.data.code == 0) {
          this.kemuOptions = res.data.data;
        }
      });
      this.$http.get('option/zhangjie/zhangjie').then(res => {
        if (res.data.code == 0) {
          this.zhangjieOptions = res.data.data;
        }
      });
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.kemu != '') searchWhere.kemu = this.formSearch.kemu;
        if (this.formSearch.zhangjie != '') searchWhere.zhangjie = this.formSearch.zhangjie;
        if (this.curFenlei != '全部') searchWhere.zhangjie = this.curFenlei;
        this.$http.get('shitiku/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/shitikuDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
